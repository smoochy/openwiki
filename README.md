...
- **Coding-agent integrations** for IBM Bob, Codex, Claude Code, OpenCode, Cursor, and Kiro, using the host's model and repository tools.
...
- **OpenWiki integrations:** run OpenWiki directly inside IBM Bob, Codex, Claude Code, OpenCode, Cursor, or Kiro, using the coding agent's authenticated model and native repository tools while OpenWiki manages the documentation lifecycle.
...
openwiki integrations install <bob|codex|claude|opencode|cursor|kiro> [--project [path]]
openwiki integrations uninstall <bob|codex|claude|opencode|cursor|kiro> [--project [path]]
